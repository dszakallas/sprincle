...
BOOST_AUTO_TEST_CASE( TrimmerTestCase_0 ) {
    ...
    auto trimmer_actor = spawn_map_node<InputChange>(removeLast, self, primary::value);

    self->link_to(trimmer_actor);

    self->send(trimmer_actor, primary::value, changesIn);

    // check trimmer_actor's response
    self->receive(
      on<primary, delta<OutputChange>>() >> [&](primary, const delta<OutputChange>& changesOut){
